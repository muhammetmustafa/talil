<?php
	...
	else
	{
		try
		{
			...
			//tur_id = Resim, jpeg
			//durum_id = Değiştirilme aşamasında
			//degerlerPOST = false; değerleri post dan alma
			Mysql::sql($talil)
			->insert("dosyalar")
			->sutunlar("test_id, yukleyen_id, tur_id, kok_dizin, dizin, ad, genislik, yukseklik, durum_id, eklenme_tarihi")
			->degerler( array($tid, Oturum::al("id"), 1, $dizinTalil, $dizinOrjinal, $yukleme->alAd(), 
							   $yukleme->alGenislik(), $yukleme->alYukseklik(), 5, $tarihDamgaVeritabani) )
			->calistir();
			
			Mysql::sql($talil)
			->insert("dosyalar")
			->sutunlar("test_id, yukleyen_id, tur_id, kok_dizin, dizin, ad, genislik, yukseklik, durum_id, eklenme_tarihi")
			->degerler( array($tid, Oturum::al("id"), 1, $dizinTalil, $dizinParmak, $kucukResim->alAd(),  
							   $kucukResim->alGenislik(), $kucukResim->alYukseklik(), 5, $tarihDamgaVeritabani) )
			->calistir();
			
			Mysql::sql($talil)
			->insert("dosyalar")
			->sutunlar("test_id, yukleyen_id, tur_id, kok_dizin, dizin, ad, genislik, yukseklik, durum_id, eklenme_tarihi")
			->degerler( array($tid, Oturum::al("id"), 1, $dizinTalil, $dizinArama, $aramaResim->alAd(), 
								$aramaResim->alGenislik(), $aramaResim->alYukseklik(), 5, $tarihDamgaVeritabani) )
			->calistir();
